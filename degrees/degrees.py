...
def shortest_path(source, target):
    # Since this problem relies on connections, I believe BFS is the best approach since the problem needs an optimal solution
    ...
    explored = set()
    # Create loop to perform the BFS
    while True:
        # Check if frontier is empty, if so return None
        if frontier.empty():
            return
        
        # Pop node to explore and add ID to set of existing IDs
        currNode = frontier.remove()
        explored.add(currNode.state)

        # Check for win condition, if valid, create list of actions
        if currNode.state == target:
            # Create my list of actions as well as keep track of current working node
            actions = []
            node = currNode
            # Create a loop to append all actions
        ...
        # Create all child nodes, with state = ActorID, parent = currNode and action = Movie
        neighbors = neighbors_for_person(currNode.state)

        # Create loop to add all pairs
